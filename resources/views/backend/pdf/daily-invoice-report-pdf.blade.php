				...
				<table>
					<tbody>
						<tr>
							<td width="25%"></td>
							<td>
								<u><strong style="font-size: 16px">Daily Invoice Report ({{date('d-m-Y',strtotime($start_date))}} - {{date('d-m-Y',strtotime($end_date))}})</strong></u>
							</td>
							<td></td>
						</tr>
					</tbody>
				...
				<table border="1" width="100%">
                  ...
                  <tbody>
                    ...
                    <tr style="text-align: center;">
                      ...
                      <td style="text-align: center;">{{$invoice->approved_by_name}}</td>
                      <td style="text-align: center;">
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details['purchases']['buying_qty']}}
                      	@endforeach
                      </td>
                      <td style="text-align: center;">
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details['purchases']['unit_price']}}
                      	@endforeach
                      </td>
                      <td style="text-align: center;">
                      	@php
                      	...
                      	@endphp
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details['purchases']['buying_price']}}<br>
                      	@php
                      ...
                      <td style="text-align: center;">
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details->selling_qty}}<br>
                      	@endforeach
                      </td>
                      <td style="text-align: center;">
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details->unit_price}}<br>
                      	@endforeach
                      </td>
                      <td style="text-align: center;">
                      	@php
                      	...
                      	@endphp
                      	@foreach($invoice['invoice_details'] as $key => $details)
                      	{{$details->selling_price}}<br>
                      	@php
                      	$total_selling_price += $details->selling_price;
                      	@endphp
                      	@endforeach
                      </td>
                      <td style="text-align: center;">{{$total_selling_price}}</td>
                      <td style="text-align: center;">{{$invoice['payment']['discount_amount']}}</td>
                    ...
                    <tr>
                    	...
                    	<td style="text-align: center;">{{$discount_sum}}</td>
